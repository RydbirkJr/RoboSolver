...
public class GraphSolver implements IGameSolver {
        ...
        while(!queue.isEmpty()){
            ...
            for(RobotState robot : gameState.states.values()){
                ...
                for(Edge edge : graph[robot.row][robot.col].edges){
                    ...
                    if("11:12;;5:15;15:5".equals(lookup)){
     ...
     * This should be handled by the graph itself, ensuring no robot can access a field where another stands.
    ...
    public void placeRobot(Vertex[][] graph, Field[][] fields, int row, int col) {
        Vertex robotVertex = graph[row][col];
        ...
        outerloop:
        for (Iterator<Edge> outerIterator = robotVertex.edges.iterator(); outerIterator.hasNext(); ) {
            Edge edge = outerIterator.next();
            directions.remove(edge.direction);
            Vertex newChild = getNextVertex(row, col, graph, edge.direction);
            Vertex curVertex = edge.child;
            Field curField = fields[curVertex.row][curVertex.col];

            //In case the next field is an robot, we have to start there.
            if (curField.canMove(edge.direction)) {
                curVertex = getNextVertex(curVertex.row, curVertex.col, graph, edge.direction);
            }

            boolean isSameVertex;
            Direction oppositeDirection = getOppositeDirection(edge.direction);

            do {
                isSameVertex = (newChild.row == curVertex.row && newChild.col == curVertex.col);

                for (Iterator<Edge> innerIterator = curVertex.edges.iterator(); innerIterator.hasNext(); ) {
                    Edge innerEdge = innerIterator.next();

                    if (innerEdge.direction == oppositeDirection) {
                        if (isSameVertex) {
                            innerIterator.remove();
                            continue outerloop;

                        } else {
                            innerEdge.child = newChild;

                        }
                    }
                }

                curVertex = getNextVertex(curVertex.row, curVertex.col, graph, oppositeDirection);

            } while (!isSameVertex);
        }
        //At this point, we know that the given edge is not pointing to a neighbor vertex.
        //Thus, all vertexes between newChild and child has to be updated.

        //We need to validate if any edges is missing because of a robot right next to.
        for(Direction dir : directions){
            if(fields[row][col].canMove(dir)){
                Vertex v = getNextVertex(row, col, graph, dir);
                Edge e = getEdge(v,getOppositeDirection(dir));
                v.edges.remove(e);
            }
        }
    }
