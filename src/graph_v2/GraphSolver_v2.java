...
public class GraphSolver_v2 implements IGameSolver {
    ...
    public GameResult solveGame(Game game) {
        ...
        while(!queue.isEmpty()){
            ...
            BfsNode result = applyBfsSearch(graph.getVertex(GR.startField.row,GR.startField.row), bestResult - gameState.moves, minMoves);
        ...
        while (!queue.isEmpty()){
            ...
            for(Edge edge : node.vertex.edges){
                ...
                if(temp == null){
                    ...
                    //in case goal is reached
                    if(temp.vertex.row == goal.row && temp.vertex.col == goal.col) break;
